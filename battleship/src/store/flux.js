    ...
    actions: {
      action: (x, y) => {
          ...
          default:
            ...
            if (getActions().checkSunken(number)) {
              ...
              getStore().board.map((element, index) => {
                getStore().board[index].map((thing, bindex) => {
                  shipPosition.map((position) => {
                    console.log(`${position}, ${index},${bindex}`);
                    if (position[0] === index && position[1] === bindex)
                      brd[bindex][index] = 3;
                    return true;
                  });
                  return true;
        ...
        getStore().board.map((element, index) => {
          getStore().board[index].map((thing, bindex) => {
            if (
              getStore().board[index][bindex] === 0 ||
              getStore().board[index][bindex] === 3 ||
              getStore().board[index][bindex] === 2
            )
              successCounter += 1;
            return true;
          });
